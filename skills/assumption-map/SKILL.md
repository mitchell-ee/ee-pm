...
- **Native Miro toolchain** (same as `story-map` and `opportunity-tree`):
  ...
  - REST helper scripts for board copy: `${CLAUDE_PLUGIN_ROOT}/scripts/miro-copy-board.sh` (for throwaway test-harness boards). Connectors are **not used** in the base assumption map; the optional cross-board connector to the OST is a stretch and would use `${CLAUDE_PLUGIN_ROOT}/scripts/write-connectors.sh` if added. (These shared Miro REST scripts live in `${CLAUDE_PLUGIN_ROOT}/scripts/` and are used by all three board skills.)
...
1. Read all `product/context/opportunity-solution-tree/assumptions/assumption-*.md` files; filter by `Parent Solution: SOL-{NN}`.
2. Create a new Miro board, or reuse an existing one keyed by `solution_ref` in `product/context/assumption-maps/SOL-{NN}-{slug}/miro-metadata.json`.
3. Place board chrome via `layout_create` in this order: title (upper-left), x-axis line, y-axis line, 4 axis-tip labels, 6 legend swatches, 6 legend labels. See "Miro layout" above for exact positions and the canonical write forms (explicit defaults, matching-color borders).

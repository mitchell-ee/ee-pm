...
- **Root** — single project-level node anchoring the tree at column 0. Its `ref_id` is `PRODUCT-{SLUG}`, where `{SLUG}` names the product (uppercase, e.g. `PRODUCT-ACME`); it is the one ref-id that is not numbered, since there is only ever one. Absorb identifies it by `type: root`, not by name. Larger (~360×160), peach fill (`#ffd6cc`) so it reads as a parent layer over the white background. Vertically centered on the leaf-range midpoint. Optional — skip when the tree has only one outcome.

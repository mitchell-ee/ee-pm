...
- **Board connectors (current)** — read via the **same** `layout_read` call: at board (and frame) scope the layout DSL emits each connector as a `CONNECTOR` line whose `from`/`to` are the endpoint items' Miro URLs. No separate REST call and no second credential — connectors come back in the one board snapshot alongside the shapes. (Load the DSL grammar once with `layout_get_dsl` if the `CONNECTOR` syntax needs confirming.)
...
**Connectors are undirected edges.** Each `CONNECTOR` line carries `from` and `to`, but these reflect *the gesture* — which endpoint the user started dragging from — not tree direction. Absorb derives parent-child orientation from x-column position, not from `from`/`to`. Two endpoints in adjacent columns form a normal tree edge regardless of the line's direction.
...
Board writes go entirely through the official MCP's `mcp__miro-official__layout_update` — shapes and connectors alike, one credential. Shape content / fill / position changes edit the item's DSL line; connector create / update / delete adds, edits, or removes a `CONNECTOR` line (the layout DSL has a first-class `CONNECTOR` type). `layout_update` re-serializes the entire board to DSL on each call — story-map item 17 documents a parser bug triggered by literal `\n` in sticky content, but OST shapes use `<br />` line breaks and don't trip it.
...
| **Skip-level edge** (e.g., OUTCOME ↔ SOL) | "Should this be re-routed through an intermediate node, or did you mean to drop the SOL/OPP layer here?" | Re-route: `layout_update` to remove the offending `CONNECTOR` line (replace with empty) and add the two correct `CONNECTOR` lines. Drop layer: leave the connector and reclassify one endpoint (rare; usually wrong). |
| **Multiple-parent edge** (two incoming tree connectors) | "Which parent is correct?" | `layout_update` to remove the rejected edge's `CONNECTOR` line. |

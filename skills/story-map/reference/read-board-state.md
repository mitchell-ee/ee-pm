...
Call `layout_read` (official Miro MCP) with the `board_id` from the sidecar. It returns the full top-level item list — stickies, text, shapes (rounded rectangles in particular), frames — each with ID, position, content, and fill color. Story maps carry no connectors, so no connector read is needed; if a future map adds them, `layout_read` already emits `CONNECTOR` lines at board scope (same call), so no extra read path is required.

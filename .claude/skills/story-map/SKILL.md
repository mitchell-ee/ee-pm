...
- Official Miro MCP at `mcp.miro.com`: `mcp__miro-official__layout_create` (build the board + items), `mcp__miro-official__layout_read` (round-trip read for absorb), `mcp__miro-official__layout_update` (refresh-mode mutations), `mcp__miro-official__context_get` (board metadata). The layout DSL has no connector type, but story maps carry no connectors, so none is needed (release horizons are thin rectangles, not edges).
